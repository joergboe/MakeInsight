...
$(info -     $(var1:.c=.o))
$(info -§3d- complete match required)
$(info $$(patsubst ab%,AB%,abc abcccc -abcd) = $(patsubst ab%,AB%,abc abcccc -abcd))
$(info -§3e- Wildcard stands also for zero characters)
$(info $$(patsubst ab%,AB%,ab abc abcd) = $(patsubst ab%,AB%,ab abc abcd))
$(info $$(patsubst %ab,%AB,ab 1ab 01abcd) = $(patsubst %ab,%AB,ab 1ab 01abcd))
$(info -§3f- Escape the first literal % in pattern and replacement with \)
$(info $$(patsubst \%ab%,\%AB%,%ab %abc %abcccc %ab%aaa) = $(patsubst \%ab%,\%AB%,%ab %abc %abcccc %ab%aaa))
$(info -§3g- Only the first ‘%’ in the pattern and replacement is treated this way)
$(info $$(patsubst %ab%,%AB%,ab% 0ab% 01ab% %ab%aaa) = $(patsubst %ab%,%AB%,ab% 0ab% 01ab% %ab%aaa))
$(info -§3h- Backslashes before % must be duplicated in pattern and replacement)
$(info $$(patsubst \\%ab,\\%AB,\ab \0ab \01ab \\%ab) = $(patsubst \\%ab,\\%AB,\ab \0ab \01ab \\%ab))
$(info $$(patsubst \\\%ab\\%,\\\%AB\\%,\%ab\ \%ab\c \%ab\cd \%ab\%) = $(patsubst \\\%ab\\%,\\\%AB\\%,\%ab\ \%ab\c \%ab\cd \%ab\%))
...
$(info $$(findstring cd,abcde) = '$(findstring cd,abcde)')
$(info $$(findstring cd,a b c d e) = '$(findstring cd,a b c d e)')
...
$(info $$(filter ab%,a ab abc ab%) = '$(filter ab%,a ab abc ab%)')
$(info )
